...
def _tensor_matrix_multiply(
    ...
    * Inner loop should have no global writes, 1 multiply.
    ...
    # # out[n, i, j] = sum of dim K a[n_a, i, k] * b[n_b, k, j]
    # for pos in prange(len(out)):
    #     i = pos // (out_shape[2] * out_shape[1])
    #     j = (pos % (out_shape[2] * out_shape[1])) // out_shape[2]
    #     k = pos % out_shape[2]
        ...
        for i2 in prange(out_shape[1]):
            for i3 in prange(out_shape[2]):
                ...
                for _ in range(a_shape[2]):
                    ...
                    a_inner += a_strides[2]
                    b_inner += b_strides[1]
                out_pos = (i1 * out_strides[0] + i2 * out_strides[1] + i3 * out_strides[2])
                out[out_pos] = acc
