...
def shape_broadcast(shape1: UserShape, shape2: UserShape) -> UserShape:
    ...
    # applied rule 3, add 1s to the start of the shorter shape
    if len(shape1) > len(shape2):
        shape2 = [1 for _ in range(max_shape - len(shape2))] + list(shape2)
    else:
        shape1 = [1 for _ in range(max_shape - len(shape1))] + list(shape1)
    ret_shape = []
    for i in range(max_shape):
        # applied rule 1
        if shape1[i] != shape2[i] and 1 not in [shape1[i], shape2[i]]:
